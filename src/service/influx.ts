...
export const deleteInfluxData = async (
  start: Date,
  stop: Date,
  predicate: string,
  ...
  const deleteEndpoint = `${selectedDs.serverAddress}/api/v2/delete?org=${encodeURIComponent(
    selectedDs.orgName
  ...
  return axios
    .post(deleteEndpoint, data, config)
    .then(() => {
...
export const getJsonData = async (measurement: string, idData: string): Promise<any> => {
  try {
    ...
    console.log(result, 'qui ci interessa');
    return result;
  } catch (error) {
    console.log('ERROR DURANTE IL CARICAMENTO', error);
    throw error;
...
export const deleteDeviceEnergyData = async (
  from: Date,
  to: Date,
  measurement: string,
  idDevice: string

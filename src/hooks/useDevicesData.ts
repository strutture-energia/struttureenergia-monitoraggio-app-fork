...
import {
  _createVerificationNodes,
  createNewTreeNode,
  createNewUnionNode,
  deleteCreatedDevice,
  getAllDevicesByPeriod,
  getAvailableDevices,
  isTreeValid,
  makeFluxAnalisis,
  moveAllNodeChildrenToList,
  setActualUnionNodeValues,
} from '../service/deviceService';
import {
  getPeriodFromLocalStorage,
  saveFluxAnalysisToLocalStorage,
  savePeriodToLocalStorage,
  saveTreeDataToLocalStorage,
...
interface IuseDevicesData {
  ...
  moveToTree: (deviceIndex: number) => void;
  createUnionNode: (value: number) => void;
  moveToList: (
    treeNode: TreeItem,
...
export default function useDevicesData(): IuseDevicesData {
  ...
  const moveToList = React.useCallback(
    (treeNode: TreeItem, path: Array<number | string>, getNodeKeyCallBack: ({ treeIndex }: any) => any) => {
  ...
  const _loadDevicesByPeriod = React.useCallback(
    async (_period: Period): Promise<any[]> => {
  ...
  const onPeriodChange = React.useCallback(
    async (period: Period, _treeData: TreeItem[]): Promise<void> => {
    ...
    [setCurrentPeriod, updateDevicesList, updateFluxAnalisis, updateTreeData, _loadDevicesByPeriod, updateSankeyFrame]
  );
  ...
  const deleteDevice = React.useCallback(
    async (idDevice: string): Promise<void> => {

...
export default function CreateDeviceByCSVDialog({ open, onClose, onSave }: CreateDeviceByCSVDialogInterface) {
  ...
  const canSubmit: boolean =
    csvData && csvData.length > 0 && !!deviceName && !!idDevice && !!userId && !!area && !!userId;
  ...
  const onSubmit = async () => {
    try {
      ...
      createNewDeviceByData(deviceData)
        .then(() => {
          onSave(deviceData);
        })
  ...
  const renderFileType = () => (
    <FormControl sx={{ p: 2 }}>
      ...
      >
        {CSV_FILE_TPYES.map((ft) => (
          <FormControlLabel key={ft.value} value={ft.value} control={<Radio />} label={ft.title} />
        ))}
      </RadioGroup>
    </FormControl>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Stack pl={2}>
          <Typography variant="body1" mb={0.5} mt={1}>
            Seleziona file CSV
          </Typography>
          <CSVReader
            accept=".csv"
        ...
        <Stack flexDirection={'row'} alignItems={'center'} marginTop={'auto'} justifyContent={'flex-end'} gap={3}>
          {invalidUserId && (
          ...
          <Button disabled={!canSubmit || loading} onClick={onSubmit} sx={{ minWidth: '150px' }} variant="contained">

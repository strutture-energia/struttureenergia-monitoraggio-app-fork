...
//componente che mostra la lista dispositivi
export default function DevicesLeftSection() {
  ...
  const onPeriodChangeClick = (event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const renderHeader = () => (
    //prende i dati dal modale del calendario e li mostra come stringa
  ...
  const onDeviceDelete = async (device: Device) => {
    try {
      if (
        confirm(
          'L eliminazione del dispositivo è permanente e non potrà essere annullata. Tutti i dati associati al dispositivo andranno persi definitivamente'
        )
      ) {
        // elimina
  ...
  //render dei dispositivi disponibili
  return (
    <React.Fragment>
      <Box
        p={2}
      ...
      >
        ...
        <Stack flexGrow={1}>
          ...
          >
            {
              // map della lista di device
              devicesList.map((device: Device, i: number) => {
                const DevIcon = device.icon ? DEVICE_ICONS_SET[device.icon as DeviceIcon] : null;
                return (
                  <Stack
                    position={'relative'}
                    width={'100%'}
      ...
      {modalOpen && <CreateDeviceByCSVDialog onSave={onModalSubmit} open={modalOpen} onClose={onModalClose} />}
    </React.Fragment>
  );
}
